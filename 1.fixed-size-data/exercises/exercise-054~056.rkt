...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname exercise-054~056) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
; - "resting"
; - [-3, -1]区间的整数
; - [0, WORLD-HEIGHT]区间的整数
; "resting"表示火箭停在地面
; [-3, -1]表示倒计时
; [0, WORLD-HEIGHT]表示火箭飞行时的高度
; 高度指的是画布顶部与火箭参考点的距离
...
(define (show x)
  (place-image/align ROCKET
                     ROCKET-X (if (and (number? x) (>= x 0)) x WORLD-HEIGHT)
                     "center" "bottom"
                     (draw-background x)))
...
(check-expect (fly -1) WORLD-HEIGHT)
(check-expect (fly WORLD-HEIGHT) (- WORLD-HEIGHT SPEED))
(check-expect (fly 32) (- 32 SPEED))
(check-expect (fly 0) 0)
(check-expect (fly 2) 0)
; --- -- ---
(define (fly x)
  (cond
    [(string? x) x]
